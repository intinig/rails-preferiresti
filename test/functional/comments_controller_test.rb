...
class CommentsControllerTest < ActionController::TestCase
  ...
  def test_should_update_comment
    login_as :admin
    put :update, :would_you_prefer_id => would_you_prefers(:would_you_prefer_one).id, :id => comments(:would_you_prefer_one).id, :comment => { :body => 'ok' }
    assert_redirected_to comment_path(assigns(:comment))
  end
  ...
  def test_should_destroy_comment
    ...
    assert_difference('Comment.count', -1) do
      delete :destroy, :would_you_prefer_id => would_you_prefers(:would_you_prefer_one).id, :id => comments(:would_you_prefer_one).id
    end
